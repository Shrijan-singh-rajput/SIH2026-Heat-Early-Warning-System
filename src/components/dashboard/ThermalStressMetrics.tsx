...
const ThermalStressMetrics = ({ metrics }: ThermalStressMetricsProps) => {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-gray-900 mb-4 dark:text-gray-100">Human Thermal Stress</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* UTCI */}
        <Card className="border-l-4 border-l-purple-500">
          <div className="flex items-start justify-between mb-3">
            <div>
              <h3 className={`${TYPOGRAPHY.cardTitle} text-purple-900 dark:text-purple-300`}>UTCI</h3>
              <p className="text-xs text-gray-600 mt-0.5 dark:text-gray-400">Universal Thermal Climate Index</p>
            </div>
            <Activity className="h-5 w-5 text-purple-600 dark:text-purple-400" aria-hidden="true" />
          </div>
          <div className="space-y-2">
            <div>
              <span className="text-3xl font-bold text-gray-900 dark:text-gray-50">{metrics.utci.toFixed(1)}</span>
              <span className="text-lg text-gray-500 ml-1 dark:text-gray-400">°C</span>
            </div>
            <RiskBadge level={metrics.utciRisk} size="md" />
            <p className="text-sm text-gray-700 mt-2 dark:text-gray-300">{utciConfig.description}</p>
          </div>
        </Card>
        ...
        <Card className="border-l-4 border-l-orange-500">
          <div className="flex items-start justify-between mb-3">
            <div>
              <h3 className={`${TYPOGRAPHY.cardTitle} text-orange-900 dark:text-orange-300`}>WBGT</h3>
              <p className="text-xs text-gray-600 mt-0.5 dark:text-gray-400">Wet Bulb Globe Temperature</p>
            </div>
        ...
        <Card className="border-l-4 border-l-red-500">
          <div className="flex items-start justify-between mb-3">
            <div>
              <h3 className={`${TYPOGRAPHY.cardTitle} text-red-900 dark:text-red-300`}>Heat Index</h3>
              <p className="text-xs text-gray-600 mt-0.5 dark:text-gray-400">Apparent Temperature</p>
            </div>
